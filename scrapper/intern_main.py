...
class JobScrapingOrchestrator:
    def __init__(self, base_url="https://freshercareers.in/category/internships/"):
    ...
    def load_existing_data(self):
        ...
        self.csv_file = module_dir / "internships_final.csv"
    ...
    def process_all_jobs_with_ai(self):
        ...
        for i, job in enumerate(self.detailed_jobs_data, 1):
            ...
            company_name = (job.get('company_name') or 'hiring').strip().split()[0].lower()
            image_path = f"{company_name}.png"
    ...
    def save_final_csv(self, filename=None):
        ...
        if filename is None:
            module_dir = Path(__file__).resolve().parent
            filename = module_dir / "internships_final.csv"
